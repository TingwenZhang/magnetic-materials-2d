...
def test_hyper_search_2d_raises_value_error_for_wrong_param_grid():
    """
    hyper_search_2d must raise ValueError if param_grid has not exactly two keys.
    """
    X = np.array([[0], [1]])
    y = np.array([0, 1])
...
def test_best_hyperparameters_single_model_within_custom_ranges():
    """
    When only one model class is passed, best_hyperparameters should return a tuple (best_p1, best_p2),
    and both values must come from the provided custom_ranges.
    """
...
def test_best_hyperparameters_multi_model_returns_dict_and_values_in_range():
    """
    When passing two model classes, best_hyperparameters should return a dict mapping
    each model’s __name__ to a (best_p1, best_p2) tuple. Each entry must lie within
    our custom_ranges.
    """
    df = make_tiny_dataframe()
    ...
    assert set(result.keys()) == {"RandomForestRegressor", "ExtraTreesRegressor"}
...
def test_best_hyperparameters_missing_custom_range_raises_key_error():
    """
    If tune_params names are not in defaults and custom_ranges is None (or missing the key),
    best_hyperparameters should raise KeyError.
    """
    df = make_tiny_dataframe()
