...
# Use Agg backend so no GUI is required for plotting
matplotlib.use("Agg")
...
def test_best_descriptors_handles_perfect_predictor(monkeypatch):
    """
    best_descriptors uses threshold=0.0 first (f1 & f2 both included => R²=1.0),
    so it will return ['f1', 'f2'] rather than just ['f1'].
    """
    df = make_multi_feature_df()
